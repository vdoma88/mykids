...
// Значок в трее помощник обычно только рисует: подпись и меню приходят от
// службы вместе с решением. Но два состояния службе неизвестны, потому что
// это состояния разговора с ней, а не экранного времени, — и их помощник
// называет сам.

// StartingTray — значок до первого ответа службы.
//
// Нужен потому, что пустой трей в первые секунды после входа в систему
// выглядит как программа, которая не запустилась. Числа здесь не выдумываем:
// сколько осталось, помощник не знает и знать не должен.
func StartingTray() screen.Tray {
	return screen.Tray{
		Tip:   "MyKids · запускается",
		Items: []screen.Item{{Label: "Запускается"}},
	}
}

// TrayOnServiceLost — значок, когда службы не слышно.
//
// Подпись не должна замереть на последнем известном остатке: он устаревает с
// каждой секундой, а выглядит как свежий. «Нет связи» честнее — и это то же,
// что написано на экране, если молчание затянулось.
func TrayOnServiceLost(since time.Duration) screen.Tray {
	return screen.Tray{
		Tip: "MyKids · нет связи со службой",
		Items: []screen.Item{
			{Label: fmt.Sprintf("Нет связи со службой — уже %s", screen.Duration(since))},
		},
	}
}
