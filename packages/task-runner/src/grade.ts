...
/**
 * Сколько кредитов заслуживает такой ответ — до дневных потолков, cooldown и
 * затухания за повтор: их считает домен.
 *
 * Правило одно на всех: раннер показывает сумму сразу, сервер начисляет её же.
 * Две копии этой арифметики расходятся молча — и разошлись: страница заданий
 * платила полную цену за частично верный ответ и полную же цену за задание,
 * которое ещё ждёт подтверждения родителя.
 */
export function earnedCredits(item: TaskItem, verdict: Grade, base: number): number {
  ...
  if (isScored(item.type)) {
    if (verdict.score <= 0) return 0;
    // Частичный зачёт даёт часть кредитов, но не меньше одного за ненулевой результат.
    return Math.max(1, Math.round(base * verdict.score));
  }
  return verdict.complete ? base : 0;
}
