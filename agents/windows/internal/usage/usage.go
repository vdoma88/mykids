...
type Accountant struct {
	...
	// Остаток меньше секунды, перенесённый на следующий замер. Копится только
	// для засчитываемого времени: простой и белый список сюда не попадают.
	carry time.Duration
}
...
func (a *Accountant) Observe(s Sample, day *Day, dayKey string) int {
	...
	// Остаток меньше секунды не выбрасываем, а копим до следующего замера.
	//
	// Округление вниз выглядит безобидно, пока замеры идут редко. Но помощник
	// шлёт наблюдения каждую секунду, и таймер службы тикает своим чередом:
	// промежутки выходят по полсекунды, каждый округляется в ноль, и час
	// экрана превращается в считанные минуты. Проверено живым запуском — за
	// сорок секунд списывалось шесть.
	a.carry += gap
	seconds := int(a.carry / time.Second)
	a.carry -= time.Duration(seconds) * time.Second
